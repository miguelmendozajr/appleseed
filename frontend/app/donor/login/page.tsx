...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/donors/login`, {
        method: 'POST',
        headers: {
      ...
      <nav className="border-b border-gray-200 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            >
              Ir al inicio de sesión de organizaciones civiles
            </Link>
          </div>
        </div>
      </nav>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                Ingresa con tu cuenta de donador
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading || !formData.rfc || !formData.contrasena}
            ...
            <div className="mt-6 text-center">
              <p className="text-gray-600">
                ¿No tienes cuenta?{' '}
                <Link href="/donor/signup" className="text-[#8BC34A] font-semibold hover:text-[#7CB342] transition-colors">
                  Regístrate aquí
                </Link>
              </p>
            </div>
